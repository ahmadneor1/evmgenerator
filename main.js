    ...
    try {
        ...
        while (count > 0) {
            ...
            if (theWallet) {
                ...
                appendFileSync(
                    './privkey.txt', `${createWalletResult.privateKey}\n`
                );
                appendFileSync(
                    './mnemonic.txt', `${createWalletResult.mnemonicKey}\n`
                );
        ...
        setTimeout(() => {
            console.log(
                chalk.green(
                    'All wallets have been created. Check your results in each file (address, mnemonic, and private key).'
                )
            );
        }, 3000);
        return;
